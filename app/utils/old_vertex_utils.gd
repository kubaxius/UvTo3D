...
func new_is_point_in_triangle(point, v1, v2, v3, tolerance = 0.001):
	var dis1 = _get_signed_distance_from_line(point, v1, v2)
	print(dis1)
...
func _get_signed_distance_from_line(point: Vector2, l_start: Vector2, l_end: Vector2):
...
func _get_uv_triangles_array(mesh:Mesh, uv_map_size:Vector2i) -> Array:
	...
	for face_id in mesh_tool.get_face_count():
		...
		face.max_x = max(face.verticies[0].uv.x, face.verticies[1].uv.x, face.verticies[2].uv.x)
		face.min_x = min(face.verticies[0].uv.x, face.verticies[1].uv.x, face.verticies[2].uv.x)
		face.max_y = max(face.verticies[0].uv.y, face.verticies[1].uv.y, face.verticies[2].uv.y)
		face.min_y = min(face.verticies[0].uv.y, face.verticies[1].uv.y, face.verticies[2].uv.y)
...
func _uv_to_3d(uv, faces, tolerance):
	for face in faces:
		var bary = is_point_in_triangle(uv, face.verticies[0].uv, face.verticies[1].uv, face.verticies[2].uv, tolerance)
		if bary != null:
			#new_is_point_in_triangle(uv, face[0].uv, face[1].uv, face[2].uv)
			var uv_3d_coords = bary2cart(face.verticies[0].location, face.verticies[1].location, face.verticies[2].location, bary)
			return uv_3d_coords
...
func _uv_to_3d_step(x, y, faces):
	...
	if map[x][y] is int:
		map[x][y] = _uv_to_3d(Vector2(x, y), faces, 0.5)
